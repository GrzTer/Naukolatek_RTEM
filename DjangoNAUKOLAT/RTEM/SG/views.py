...
class EnergyPricesView(View):
    def post(self, request):
        ...
        data_path = os.path.join(
            settings.BASE_DIR, f"SG/data/energy_prices_{country_code}.csv"
        )
    ...
    def fetch_historical_data(self, country_code="PL", start_date=None, end_date=None):
        ...
        try:
            ...
            if prices.empty:
                raise ValueError(
                    f"No data available for {country_code} from {start_date} to {end_date}"
                )
            ...
            print(
                f"An error occurred while fetching or saving data for {country_code}: {str(e)}"
            )
