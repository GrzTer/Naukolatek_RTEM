...
* **Celery** – asynchronous data ingestion and ML jobs
* **TensorFlow / Keras** – consumption forecasting models
* **WebSockets** – live dashboards and alerts
...
|-------|------------------|---------|
| `Device` | `serial_number` (auto), `building`, `location` | Uniquely identifies each monitored asset. |
| `TemperatureMeasurement` | `device` (FK), `value`, `timestamp` | Tracks thermal conditions per device. |
| `VoltageMeasurement` | `device` (FK), `voltage`, `current`, `timestamp` | Logs electrical parameters for analytics. |

The schema is designed for easy extension when new IoT sensor types are introduced.
